...
业务测试必须断言状态码、业务响应和数据变化，不能只断言路由不是 404。单独的路由注册 smoke 可以只查方法与路径装配，但必须明确标注为装配证据，不得计作权限或 CRUD 业务证据。

## G3：数据库迁移

- 空数据库可升级到最新版本。
- 已有测试数据可从上一发布版本升级。
- 对由 `metadata.create_all` 建成的存量库，先备份并比较实际结构与基线；只有守卫命令确认结构等价，才允许 `python main.py stamp --env=prod --confirm-schema-equivalent`，差异必须通过显式修复迁移处理。操作步骤见[数据库迁移运行手册](database-migration-runbook.md)。
- 关键迁移具有回滚方案；无法自动降级时要写明恢复步骤。
- 租户字段、唯一约束和索引通过数据库层验证。

当前候选状态：schema-only 基线已在全新 SQLite 与 MySQL 8.0.43 上升级成功；非空未标记数据库保护和受控 stamp 已有自动测试。真实上一发布版本数据库副本的备份、升级和恢复演练尚未完成，因此 G3 仍不能宣称完全关闭。证据见[Phase 1 审计修正验证记录](verification-2026-07-18-phase1.md)。
